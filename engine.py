...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, data in enumerate(data_loader):
        ...
        out_c4, out_c3, out_c2, out_c1, out = model(samples)
        loss_final = criterion(out_c4, targets.float()) + criterion(out_c3, targets.float()) + criterion(out_c2, targets.float()) + criterion(out_c1, targets.float()) + criterion(out, targets.float())
