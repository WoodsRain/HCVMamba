...
def main(args):
    ...
    # --------- [ADD] 保存参数快照 + 初始化指标文件（非常少量改动） ---------
    run_dir = Path(process_folder_path)
    ...
    csv_fields = ["epoch", "lr", "mean_test_loss", "mIoU", "ODS", "OIS", "F1", "Precision", "Recall", "save_root"]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data) in enumerate(test_dl):
                ...
                out_c4, out_c3, out_c2, out_c1, out = model(x)
                loss = criterion(out_c4, target.float()) + criterion(out_c3, target.float()) + criterion(out_c2, target.float()) + criterion(out_c1, target.float()) + criterion(out, target.float())
                ...
                pbar.update(1)
        pbar.close()
        mean_test_loss = float(np.mean(test_losses)) if len(test_losses) > 0 else None
